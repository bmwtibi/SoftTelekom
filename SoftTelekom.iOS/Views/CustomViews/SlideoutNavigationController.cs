...
namespace SoftTelekom.iOS.Views.CustomViews
{
    ...
    public class SlideoutNavigationController : UIViewController
    {
        ...
        public SlideoutNavigationController ()
        {
            ...
            SlideWidth = 245f;
			//SlideHeight = 44f + 20f;
            SlideHeight = (float)UIScreen.MainScreen.Bounds.Height;
        ...
        private void Pan (UIView view)
        {
			try
			{
	            if (_panGesture.State == UIGestureRecognizerState.Began) {
                    _panOriginX = (float)view.Frame.X;
	                _ignorePan = false;

	                if (!Visible) {
						if (_panGesture.NumberOfTouches == 0)
							return;
                        PointF touch = (PointF)_panGesture.LocationOfTouch(0, view);
                    ...
                    float t = (float)_panGesture.TranslationInView(view).X;
	                ...
	                if ((LeftMenuEnabled && (_panOriginX + t) >= 0) || (RightMenuEnabled && (_panOriginX + t) <= 0))
                        view.Frame = new RectangleF(_panOriginX + t, (float)view.Frame.Y, (float)view.Frame.Width, (float)view.Frame.Height);
	            } else if (!_ignorePan &&
	                (_panGesture.State == UIGestureRecognizerState.Ended ||
	                _panGesture.State == UIGestureRecognizerState.Cancelled)) {
                        float velocity = (float)_panGesture.VelocityInView(view).X;
	                ...
	                if (Visible) {
						if ((view.Frame.X < (SlideWidth / 2) && _leftMenuShowing) || (view.Frame.X > -(SlideWidth / 2) && _rightMenuShowing))
	                        ...
	                        UIView.Animate (SlideSpeed, 0, UIViewAnimationOptions.CurveEaseInOut,
	                                        () => {
                                                view.Frame = new RectangleF(SlideWidth, (float)view.Frame.Y, (float)view.Frame.Width, (float)view.Frame.Height);
	                        }, () => { });
						...
						if (velocity > 800.0f || (view.Frame.X > (SlideWidth / 2))) {
	                        ...
	                        UIView.Animate (SlideSpeed, 0, UIViewAnimationOptions.CurveEaseInOut,
	                                        () => {
                                                view.Frame = new RectangleF(0, 0, (float)view.Frame.Width, (float)view.Frame.Height);
                                            }, () => { });
	                    }
	                }
	            }
        ...
        public override void ViewDidLoad ()
        {
            ...
            _internalTopView.View.Frame = new RectangleF(0, 0, (float)View.Frame.Width, (float)View.Frame.Height);
            _internalMenuViewLeft.View.Frame = new RectangleF(0, 0, SlideWidth, (float)View.Frame.Height);
            _internalMenuViewRight.View.Frame = new RectangleF((float)View.Frame.Width - SlideWidth, 0, SlideWidth, (float)View.Frame.Height);
        ...
        public void ShowMenuLeft ()
        {
            ...
            _internalMenuViewLeft.View.Frame = new RectangleF(0, 0, SlideWidth, (float)View.Bounds.Height);
			if (MenuViewLeft != null)
				MenuViewLeft.ViewWillAppear(true);

            UIView view = _internalTopView.View;
			UIView.Animate (SlideSpeed, 0, UIViewAnimationOptions.CurveEaseInOut,
                () => view.Frame = new RectangleF(SlideWidth, 0, (float)view.Frame.Width, (float)view.Frame.Height),
                           () =>
            {
        ...
        public void ShowMenuRight ()
        {
            ...
            _internalMenuViewRight.View.Frame = new RectangleF((float)View.Frame.Width - SlideWidth, 0, SlideWidth, (float)View.Bounds.Height);
        ...
        {
            return new UIBarButtonItem(UIImage.FromBundle("SoftTelekomResources/Images/leftMenu"), UIBarButtonItemStyle.Plain, (sender, args) => ShowMenuLeft())
            {
        ...
        {
            return new UIBarButtonItem(UIImage.FromBundle("SoftTelekomResources/Images/rightMenu"), UIBarButtonItemStyle.Plain, (sender, args) => ShowMenuRight())
            {
                ...
                View =
                                             {
                                                 Frame = new RectangleF(0, 0,
                                                                        (float)_internalTopView.View.Frame.Width,
                                                                        (float)_internalTopView.View.Frame.Height)
                                             }
            };
